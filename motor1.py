...
class MotorConfig:
    ...
    def __init__(self):
        ...
        # 低速三轮统一映射：峰值 < 4 cm/s 视为停止；4--8 cm/s
        # 等比抬升至峰值 8 cm/s，以保持全向运动方向不变。
        self.wheel_speed_deadband_cm_s = 4.0
        self.minimum_active_wheel_speed_cm_s = 8.0
...
class MotorSystem:
    ...
    def _set_duty(self, duty_1, duty_2, duty_3):
        ...
        if self._motors is not None:
            # EN/PH反接，原范围 (-10000, 10000) 映射至 (0, 10000)
            self._motors[0].duty(int((duty_1 + 10000) / 2))
            self._motors[1].duty(int((duty_2 + 10000) / 2))
            self._motors[2].duty(int((duty_3 + 10000) / 2))
